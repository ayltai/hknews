...
public final class ItemsHandler extends ApiHandler {
    ...
    public APIGatewayProxyResponseEvent handleRequest(@NotNull final APIGatewayProxyRequestEvent event, @NotNull final Context context) {
        ...
        final int                 pageNumber    = Integer.parseInt(queries.getOrDefault("pageNumber", String.valueOf(Configuration.DEFAULT_PAGE_NUMBER)));
        final int                 pageSize      = Integer.parseInt(queries.getOrDefault("pageSize", String.valueOf(Configuration.DEFAULT_PAGE_SIZE)));
        ...
        try {
            ...
            final Page<Item>       page       = new Page<>(pageable, sort, pageable.getPageSize(), totalPages, items.size(), pagedItems.size(), pageable.getPageSize() == 1, pageable.getPageNumber() == totalPages, isEmpty, pagedItems);
