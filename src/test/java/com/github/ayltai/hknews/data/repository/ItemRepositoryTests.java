...
public final class ItemRepositoryTests extends RepositoryTests<ItemRepository, Item> {
    ...
    protected ItemRepository createRepository() throws InterruptedException {
        return new ItemRepository(this.client, Mockito.mock(LambdaLogger.class));
    }
    ...
    public void testFindBySourceNameInAndCategoryNameInAndPublishDateAfterOrderByPublishDateDesc() {
        try {
            ...
            Assertions.assertEquals("title", repository.findBySourceNameInAndCategoryNameInAndPublishDateAfterOrderByPublishDateDesc(Collections.singletonList("sourceName"), Collections.singletonList("categoryName"), 3).iterator().next().getTitle());
